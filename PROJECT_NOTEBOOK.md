...
- **افزودن بچ جدید:** `node scripts/merge-batch.mjs <path-to-batch.json>` — دو شکل خام سؤال (canonical و flat/correct_index) را خودکار تشخیص و نُرمال می‌کند، تکراری‌ها را بر اساس id رد می‌کند، و نام‌های هم‌معنیِ مضمون/سختی را یکدست می‌کند (`SUBJECT_ALIASES`/`DIFFICULTY_ALIASES` در همان اسکریپت). بعد از اجرا، `BANK_VERSION` را در `bank/index.ts` یک واحد بالا ببر تا seed دوباره اجرا شود. کد اپ هرگز نیازی به تغییر ندارد.
