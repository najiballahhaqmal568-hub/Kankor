...
**به‌روزرسانی مهم (۱۴۰۵/۰۴/۲۱):** بانک سؤالات نمونه با بانک واقعی رهنمون ۱۴۰۵ جایگزین شد؛ مدل داده (schema.ts) کاملاً با ساختار `questions.json` یکی شد — نه adapter، نه فیلد اضافه. جزئیات در ADR-007. سپس بچ دوم (۱۳ سؤال) و بچ سوم (۲۱ سؤال) ریاضی، هر دو با شکل خام flat/correct_index، با `scripts/merge-batch.mjs` نُرمال و ادغام شدند؛ بانک اکنون **۹۱ سؤال** دارد (ریاضیات ۵۶، کیمیا ۱۲، فزیک ۱۱، بیولوژی ۱۲ — یکی از سؤالات ریاضی به‌دلیل ابهام در فورم اصلی `verified:false` است و فقط در dev دیده می‌شود). جزئیات در ADR-008.
