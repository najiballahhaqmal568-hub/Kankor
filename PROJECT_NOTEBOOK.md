...
**به‌روزرسانی مهم (۱۴۰۵/۰۴/۲۱):** بانک سؤالات نمونه با بانک واقعی رهنمون ۱۴۰۵ جایگزین شد؛ مدل داده (schema.ts) کاملاً با ساختار `questions.json` یکی شد — نه adapter، نه فیلد اضافه. جزئیات در ADR-007. سپس بچ‌های دوم تا پنجم (۱۳ + ۲۱ + ۹ ریاضی، و ۷ کیمیا/بیولوژی)، همه با شکل خام flat/correct_index، با `scripts/merge-batch.mjs` نُرمال و ادغام شدند؛ بانک اکنون **۱۰۷ سؤال** دارد (ریاضیات ۶۵، کیمیا ۱۸، فزیک ۱۱، بیولوژی ۱۳ — یکی از سؤالات ریاضی به‌دلیل ابهام در فورم اصلی `verified:false` است و فقط در dev دیده می‌شود). جزئیات در ADR-008.
